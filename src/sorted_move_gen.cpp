...
void SortedMoveGen<movegen::MoveGenType::CAPTURE>::set_score(Move& move){
    ...
    int score = (to_piece == Piece::NONE ? -25000 : 10*piece_value[to_piece.type()]) - 10*piece_value[from_piece.type()]
        + 10 * chk_2 * bool(check_squares[from_piece.type()] & Bitboard::fromSquare(move.to()))
        + 250 * capture_history.get(pos.sideToMove(), move.from().index(), move.to().index(), to_piece) / 10'000;
