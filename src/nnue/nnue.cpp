...
void update_accumulators(Accumulators& prev_accs, Accumulators& new_accs, BothModifiedFeatures& both_features){

    vec_int16 registers[NUM_AVX_REGISTERS];

    constexpr int CHUNK_SIZE = NUM_AVX_REGISTERS*INT16_PER_REG;

    Accumulator& prev_acc = prev_accs[0];
    Accumulator& new_acc  = new_accs[0];
    ModifiedFeatures& features = both_features[0];
    ...
    prev_acc = prev_accs[1];
    new_acc  = new_accs[1];
    features = both_features[1];
